        ...
        <div className="max-w-4xl mx-auto space-y-6">
            <div className="bg-white p-8 rounded-2xl shadow-sm border space-y-6">
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900">AI TMA Assistant</h2>
                        <p className="text-gray-500">Paste your question below for instant analysis and structure suggestions.</p>
                    </div>
                </div>
                ...
                <textarea
                    className="w-full h-48 p-4 border rounded-xl focus:ring-2 focus:ring-primary-500 transition outline-none resize-none"
                ...
                <div className="flex items-center justify-between">
                    <button
                        ...
                        className={`flex items-center space-x-2 px-4 py-2 rounded-lg transition ${hausaToggle ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                    >
            ...
            {result && (
                <div className="bg-white p-8 rounded-2xl shadow-lg border-2 border-primary-100 space-y-6 animate-in fade-in slide-in-from-bottom-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-4">
                            ...
                            <p className="text-gray-600 bg-gray-50 p-4 rounded-lg">{result.breakdown}</p>

                            <h3 className="font-bold text-gray-900 flex items-center space-x-2 pt-2">
                                <span className="w-2 h-6 bg-primary-500 rounded-full"></span>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {result.concepts?.map((c: string, i: number) => (
                                    <span key={i} className="bg-primary-50 text-primary-600 px-3 py-1 rounded-full text-sm font-medium">#{c}</span>
                                ))}
                            </div>
                            ...
                            <div className="flex items-center justify-between bg-gray-50 p-4 rounded-lg">
                                <code className="text-xs text-gray-700">{result.references}</code>
                            ...
                            {result.hausa && (
                                <div className="bg-green-50 p-4 rounded-lg border border-green-100 mt-4">
                                    <h4 className="font-bold text-green-800 text-sm mb-1 uppercase tracking-wider">Hausa Interpretation</h4>
                                    <p className="text-sm text-green-700">{result.hausa}</p>
                                </div>
                            )}
                        </div>
                    </div>
