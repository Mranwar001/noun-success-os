...
import { Loader2, Calendar, BookOpen, AlertCircle, TrendingUp } from "lucide-react";

export default function DashboardPage() {
    const [stats, setStats] = useState({
        coursesCount: 0,
        pendingTmas: 3, // Mock for now
        daysToExam: 0,
        progress: 65,
    });
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center py-24">
                ...
                <p className="text-gray-500">Synchronizing your academic data...</p>
            </div>
        );
    }
              ...
              <tbody className="divide-y dark:divide-gray-800">
                ...
                {courses.map((course) => (
                  <tr key={course.id} className="group hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="py-4">
                      {course.exam_date ? (
                        <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-black bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 uppercase tracking-wide">
                          Scheduled: {new Date(course.exam_date).toLocaleDateString()}
                        </span>
                      ) : (
                    ...
                    <td className="py-4">
                      <button className="text-xs font-black text-primary-600 dark:text-primary-400 hover:text-primary-800 dark:hover:text-primary-300 uppercase tracking-widest">Manage</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="rounded-2xl bg-primary-600 p-8 shadow-xl text-white relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="relative z-10 space-y-4">
            ...
            <p className="text-sm text-primary-100 opacity-80 leading-relaxed">
              Did you know? Students who use regular mock exams score 40% higher on average.
            </p>
          </div>
          <button className="mt-8 relative z-10 bg-white dark:bg-primary-950 text-primary-600 dark:text-primary-400 py-3 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-primary-50 dark:hover:bg-primary-900 transition-colors">
            Try a Mock Exam
          </button>
        </div>
      </div>
